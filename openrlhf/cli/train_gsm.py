...
def extract_ans_from_response(answer: str, sep):
    ...
    try:
        ...
        try:
            number = re.findall(r"\d+(?:,\d+)*(?:\.\d+)?", answer)[-1]
            return float(number)
        except:
            return float('inf')
        
class GSMRewardModel:
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer
        self.dataset = load_dataset('openai/gsm8k', 'main')['train']
        self.answer_dict = {elem['question']: extract_ans_from_response(elem['answer'], sep='###') for elem in self.dataset}

    def gsm_reward_model(self, sequences, attention_mask, answers):
        strings = self.tokenizer.batch_decode(sequences, skip_special_tokens=True)
        rewards = []
        for p, s in zip(answers, strings):
            answer = extract_ans_from_response(s.split("assistant")[1].strip(), sep='###')

            rewards.append(answer == extract_ans_from_response(p, sep='###'))
        return torch.Tensor(rewards).to(sequences.device)
...
def train(args):
    ...
    if not args.remote_rm_url:
        reward_model = GSMRewardModel(tokenizer).gsm_reward_model
    else:
        reward_model = None
